...
| `floor-contract` (live) | the poll floor against the shape it replaced: every `data-fs-floor` box is stripped, re-measured with `offsetHeight` — what 0.14.3 wrote — and the two must agree within 4px, then the tallest floor on the page is emptied and must be given back. Both faults shipped and neither is visible in a file or on a page that is only loaded: 41px short on `.cbi-section-descr` (/admin/network/dhcp), 1299px never taken off on Network → Interfaces. A box the poll refilled or replaced is not judged |
