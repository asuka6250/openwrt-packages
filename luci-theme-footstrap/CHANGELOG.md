...
- **A table judged mid-scroll is judged against its column, not the window.** For a slot nobody has measured yet — and every navigation makes fresh slots — the mid-scroll pass has to answer without reading layout, and it compared `window.innerWidth` against the cramped threshold. In the sidebar layout those differ by the sidebar: an 800px window whose content column is 520px cleared the threshold, so the table stayed unstacked at full natural width and was clipped by `.fs-main { overflow-x: clip }` until the reader held still. The width now comes from `fs-chrome`, which already memoises the shell's geometry against the density attribute — still no layout read.
