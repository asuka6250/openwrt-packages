...
- **The key/value column rule in `30-tables.css` now discriminates by a table's SHAPE (no `.th` anywhere, no row with a 3rd `.td`) instead of by `.tr.table-titles` — a narrower fix than the class-based test it replaces, but a real one.** `.tr.table-titles` is written only by `L.ui.Table`'s JS-captioned constructor (`ui.js:3808-3820`); `initFromMarkup` (`ui.js:3956-3984`) binds a plain `<thead>`/`.th` markup table and writes no such class. Live on both `owrt2512`/apk and `owrt2410`/opkg (1280 and 380px, HEAD vs worktree on the same DOM): `fs-select.js`'s `tagDataTables()` already tags any `#view` or `.modal` table carrying a header — luci-mod-dashboard's `20_lan.js`/`30_wifi.js` `<thead>`s and luci-app-acl's modal table included — with `.fs-dt`, and the key/value rule already excludes `.fs-dt`; those three read identically at HEAD and worktree, no pre-fix bug. `attendedsysupgrade`'s `11_upgrades.js` table renders via `ui.addTimeLimitedNotification` into `#maincontent`, outside `#view .cbi-section`, so it was never reached by the key/value rule either way (dividers 1px, padding `8px 12px 8px 0`, unchanged) — `pages/20-overview.css`'s move to the same shape test is for one consistent discriminator, not a user-visible fix there. The real, live effect: a table with NO header row at all and a 3rd `.td` — luci-mod-dsl's `stats.js`, the gallery's log fixture — is never tagged `.fs-dt` (`tagDataTables()`'s header check needs a header row) and used to reach the key/value rule; it now no longer stacks below 560px and drops the mono/end-aligned key/value look at desktop. And because the shape test runs in CSS alone, a header table no longer flashes key/value styling for the first frame before `fs-select.js` runs and adds `.fs-dt` — the old class-based test could only exclude it once that script ran. Verified with a Playwright sweep over the stock shapes at 1280/640/380px: stock key/value includes (`10_system.js`/`20_memory.js`/`25_storage.js`/`30_network.js`) still stack unchanged, and the two newly-excluded shapes read `[0,0,0]`. New `docs/gallery.html` fixtures (20_lan.js's `<thead>`/3-column shape, and the headerless log table, `.table.fs-xscroll`) give `computed-diff` something to measure — the log fixture switches from stacking to horizontal scroll at 390px (673 -> 900px), every other key/value fixture unchanged.
