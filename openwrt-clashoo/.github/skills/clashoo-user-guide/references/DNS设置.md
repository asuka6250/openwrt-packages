...
| IPv6 DNS | `true` | 控制内核是否支持 IPv6（写入内核的 `ipv6` 与 `dns.ipv6`）。**关闭后，凡是进代理的 IPv6 流量一律降级成 IPv4**。 | 保持开启 |
...
这三个开关经常一起出问题，放在一起说清楚。

| 开关 | 位置 | 作用 |
|---|---|---|
| IPv6 DNS | 配置 → DNS | 内核认不认 IPv6。**关掉则凡是进代理的 IPv6 流量一律降级成 IPv4** |
| 大陆 IPv4 绕过 | 系统 → 绕过规则 | 国内 IPv4 目标在防火墙层直接放行，不进内核 |
| 大陆 IPv6 绕过 | 系统 → 绕过规则 | 国内 IPv6 目标在防火墙层直接放行，不进内核 |

### 两条规矩

**一、IPv6 DNS 必须开。** 它关着而「IPv6 代理」还开着时，防火墙把 IPv6 交给内核、内核却被配置成不支持 IPv6，结果是境内外 IPv6 全部失效。这是默认值曾经的坑，现已改为默认开启。

**二、两个绕过要么都勾、要么都不勾。** 一开一关会让同一个网站的 IPv4 走直连、IPv6 走代理，对外露出两个不同的源地址，反而更容易出问题。

### 都勾选（默认）

**优点**

- 国内流量不进内核，省 CPU 和内存，延迟更低
- 国内大流量场景（4K、下载、网盘）性能明显更好
- IPv6 测试站显示设备自己的公网地址，而不是路由器的

**缺点**

- 分流依据是 GeoIP 地址段，遇到 CDN 可能误判
- 这部分连接不经过内核，面板上看不到日志和流量统计
- 依赖「Fake-IP 过滤域名」里的 `rule-set:cn_domain`
...
- 所有流量统一进内核，行为一致、可预测
- 分流按域名规则判定，比 IP 段更精准，CDN 不易误判
- 面板日志和连接统计完整

**缺点**

- 国内流量也要过一遍内核，低端设备 CPU 压力大
- 国内 IPv6 由路由器代发，测试站显示路由器地址
- 大流量场景性能不如直连

### 「都勾」成立的前提

「Fake-IP 过滤域名」里必须有 `rule-set:cn_domain`（默认带）。它让国内域名返回真实 IP，从而与游戏等应用下发的硬编码 IP 走同一条直连路径。缺了它，国内域名会走 Fake-IP 进内核，同一个应用的连接被分到两条路上，部分网游可能偶发掉线。
